...
def test_dry_run_reports_update_mode_for_existing_files(sandbox, git_stub):
    ...
    assert (
        ...
        " from ref v1.0.18" in result.stdout
    )
...
def test_update_syncs_via_consumer_git_sequence(sandbox, git_stub):
    ...
    assert log.read_text().splitlines() == [
        ...
        "--quiet",
        "-m",
        "chore(agent): update assistant files to v1.0.18",
        "--",
        ".agent",
        "agent.sh",
        ".githooks",
...
def test_update_noop_reports_already_up_to_date(sandbox, git_stub):
    ...
    assert result.returncode == 0, result.stderr
    assert "already up to date at ref v1.0.18" in result.stdout
    assert "update complete" in result.stdout
...
def test_overwrite_warning_precedes_update_actions_and_names_scope(
    ...
    assert "failed to retrieve the assistant ref v1.0.18" in result.stderr
...
def release_repo(tmp_path: Path) -> Path:
    """Local release repository tagged v1.0.18 holding the assistant
    ...
    _git(repo, "commit", "-m", "release v1.0.18")
    _git(repo, "tag", "v1.0.18")
    return repo
