...
```bash
curl -fsSL https://raw.githubusercontent.com/daveonche/ai_assistant/v1.0.14/scripts/install.sh | bash
```
...
```bash
git fetch --depth 1 https://github.com/daveonche/ai_assistant.git v1.0.14 && git show FETCH_HEAD:scripts/install.sh | bash
```
...
Installs and updates always retrieve the pinned release tag `v1.0.14`,
never `main`, so repeated runs are reproducible. To install from a
different reference, pass `--ref`:

```bash
curl -fsSL https://raw.githubusercontent.com/daveonche/ai_assistant/v1.0.14/scripts/install.sh | bash -s -- --ref v1.0.0
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/daveonche/ai_assistant/v1.0.14/scripts/install.sh | grep -E 'DEFAULT_REF=|no commits yet'
```
